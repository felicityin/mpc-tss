...
func UnmarshalJSONPoint(payload []byte) (*ECPoint, error) {
	...
	if len(aux.Curve) > 0 {
		...
		ec, ok = tss.GetCurveByName(tss.CurveName(aux.Curve))
		if !ok {
			return nil, fmt.Errorf("cannot find curve named with %s in curve registry, please call tss.RegisterCurve(name, curve) to register it first", aux.Curve)
		}
	} else {
		// forward compatible, use global ec as default value
		ec = tss.EC()
	...
	p := &ECPoint{ec, aux.Coords}

	if !p.IsOnCurve() {
		return nil, fmt.Errorf("ECPoint.UnmarshalJSON: the point is not on the elliptic curve (%T) ", p.curve)
	}
