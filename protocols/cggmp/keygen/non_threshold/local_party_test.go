...
func testE2EConcurrentAndSaveFixtures(t *testing.T, kind int) {
	...
	endCh := make(chan *save.LocalPartySaveData, len(pIDs))
...
Keygen:
	for {
		...
		case data := <-endCh:
			...
			index, err := data.OriginalIndex()
			assert.NoErrorf(t, err, "should not be an error getting a party's index from save data")
			tryWriteTestFixtureFile(t, kind, index, *data)
			...
			if atomic.LoadInt32(&ended) == int32(len(pIDs)) {
				...
				if kind == Ecdsa {
					verifyEcdsa(t, parties, data)
				} else {
					verifyEddsa(t, parties, data)
				}
...
func verifyEcdsa(t *testing.T, parties []*LocalParty, save *save.LocalPartySaveData) {
	x := new(big.Int)
...
func verifyEddsa(t *testing.T, parties []*LocalParty, save *save.LocalPartySaveData) {
	x := new(big.Int)
	for _, Pj := range parties {
		xj := Pj.data.PrivXi
...
func tryWriteTestFixtureFile(t *testing.T, kind, index int, data save.LocalPartySaveData) {
