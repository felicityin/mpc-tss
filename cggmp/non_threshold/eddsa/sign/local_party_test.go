...
func TestE2EConcurrent(t *testing.T) {
	...
	keys, signPIDs, err := keygen.LoadKeygenTestFixturesRandomSet(keygen.Eddsa, testThreshold, testParticipants)
	assert.NoError(t, err, "should load keygen fixtures")
	assert.Equal(t, testThreshold, len(keys))
	assert.Equal(t, testThreshold, len(signPIDs))

	auxs, _, err := auxiliary.LoadAuxTestFixtures(keygen.Eddsa, testThreshold)
...
func TestE2EConcurrentWithLeadingZeroInMSG(t *testing.T) {
	...
	// PHASE: load keygen fixtures
	keys, signPIDs, err := keygen.LoadKeygenTestFixturesRandomSet(keygen.Eddsa, threshold, testParticipants)
	assert.NoError(t, err, "should load keygen fixtures")
	auxs, _, err := auxiliary.LoadAuxTestFixtures(testThreshold, testParticipants)
	assert.NoError(t, err, "should load aux fixtures")
	assert.Equal(t, testThreshold, len(keys))
	assert.Equal(t, testThreshold, len(signPIDs))
